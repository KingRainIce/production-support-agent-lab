...
def audit_operations_automation_execution_row(record: OperationsAutomationExecutionRecord) -> dict[str, Any]:
    return {
        ...
        "command_summary": {
            ...
            "path_hash": audit_hash(record.command_path),
            "query_keys": sorted(str(key)[:120] for key in record.command_query.keys()),
            "query_hash": _audit_json_hash(record.command_query),
            "body_keys": sorted(str(key)[:120] for key in record.command_body_keys),
            "body_hash": record.command_body_hash,
            "fingerprint": record.command_fingerprint,
        },
        "result_summary_hash": audit_hash(record.result_summary),
        "result_summary_length": len(record.result_summary or ""),
