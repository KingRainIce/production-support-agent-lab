...
`operations_automation_executions` is also separate from `events`. It records console, cron, or on-call bot execution of server-generated automation actions with actor, action kind, status, command method/path/query summary, body key list, body hash, command fingerprint, and a bounded result summary. The admin ledger keeps raw command bodies and raw command results out of durable storage while still proving which automation was attempted and whether it succeeded. The SIEM/warehouse audit export is stricter: it exports only method, query keys, body keys, path/query/body/result hashes, fingerprints, status, and correlation hashes, not raw paths, query values, bodies, result summaries, action ids, or actors. A bounded summary endpoint and `/metrics` aggregate this ledger into an automation health signal without exposing action ids, actors, raw commands, or raw results.
